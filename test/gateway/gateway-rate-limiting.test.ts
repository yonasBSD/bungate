...
describe("BunGateway Rate Limiting (0http-bun)", () => {
  ...
  test("should apply rate limiting and return 429 when exceeded", async () => {
    ...
    for (let i = 0; i < 3; i++) {
      const request = new Request("http://localhost/api/rate-limited", { method: "GET" });
    ...
    const request4 = new Request("http://localhost/api/rate-limited", { method: "GET" });
    ...
    const errorData = (await response4.json()) as { error: string; message: string };
  ...
  test("should use custom key generator for rate limiting", async () => {
    ...
    const user1Request1 = new Request("http://localhost/api/custom-rate-limit", {
      method: "GET",
    ...
    const user1Request2 = new Request("http://localhost/api/custom-rate-limit", {
      method: "GET",
      headers: { "x-user-id": "user1" },
    });
    ...
    const user1Request3 = new Request("http://localhost/api/custom-rate-limit", {
      method: "GET",
      headers: { "x-user-id": "user1" },
    });
    ...
    const user2Request1 = new Request("http://localhost/api/custom-rate-limit", {
      method: "GET",
  ...
  test("should exclude paths from rate limiting", async () => {
    ...
    const request1 = new Request("http://localhost/api/maybe-limited/data", { method: "GET" });
    const response1 = await gateway.fetch(request1);
    expect(response1.status).toBe(200);

    const request2 = new Request("http://localhost/api/maybe-limited/data", { method: "GET" });
    const response2 = await gateway.fetch(request2);
    expect(response2.status).toBe(429);

    // But health endpoint should never be rate limited
    for (let i = 0; i < 5; i++) {
      const healthRequest = new Request("http://localhost/api/maybe-limited/health", { method: "GET" });
  ...
  test("should provide rate limit context in request", async () => {
    ...
    // First request
    const request1 = new Request("http://localhost/api/rate-context", { method: "GET" });
    const response1 = await gateway.fetch(request1);
    expect(response1.status).toBe(200);
    ...
    // Second request
    const request2 = new Request("http://localhost/api/rate-context", { method: "GET" });
    const response2 = await gateway.fetch(request2);
  ...
  test("BunGateway Rate Limiting (0http-bun) > should use custom message for rate limit exceeded", async () => {
    ...
    const response1 = await gateway.fetch(new Request("http://localhost/api/limited"));
    expect(response1.status).toBe(200);

    // Second request should be rate limited with custom message
    const response2 = await gateway.fetch(new Request("http://localhost/api/limited"));
    expect(response2.status).toBe(429);
  ...
  test("BunGateway Rate Limiting (0http-bun) > should skip rate limiting with skip function", async () => {
    ...
    const response3 = await gateway.fetch(new Request("http://localhost/api/limited"));
  ...
  test("BunGateway Rate Limiting (0http-bun) > should include standard rate limit headers", async () => {
    ...
    const response = await gateway.fetch(new Request("http://localhost/api/limited"));
    expect(response.status).toBe(200);
