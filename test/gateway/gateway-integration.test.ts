...
describe("BunGateway Integration", () => {
  ...
  test("should handle path parameters", async () => {
    ...
    const data = (await response.json()) as { id: string; name: string; query: any };
  ...
  test("should handle POST requests with JSON body", async () => {
    ...
    const data = (await response.json()) as { received: any; method: string; timestamp: number };
  ...
  test("should handle concurrent requests", async () => {
    const promises = Array.from({ length: 10 }, (_, i) => fetch(`${baseUrl}/health`).then((r) => r.json()));
