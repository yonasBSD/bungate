...
describe("IP-Hash Load Balancer E2E Tests", () => {
  ...
  beforeAll(async () => {
    ...
    gateway.addRoute({
      ...
      loadBalancer: {
        strategy: "ip-hash",
        targets: [{ url: "http://localhost:8120" }, { url: "http://localhost:8121" }, { url: "http://localhost:8122" }],
        healthCheck: {
          enabled: true,
          interval: 2000,
          timeout: 1000,
    ...
    gateway.addRoute({
      ...
      loadBalancer: {
        strategy: "ip-hash",
        targets: [{ url: "http://localhost:8120" }, { url: "http://localhost:8121" }],
        healthCheck: {
          enabled: true,
          interval: 2000,
          timeout: 1000,
  ...
  test("should distribute different IP addresses across servers", async () => {
    ...
    for (let i = 0; i < 3; i++) {
      const consistentResponse = await fetch("http://localhost:3005/api/ip-hash-two/test");
  ...
  test("should handle different HTTP methods consistently", async () => {
    ...
    // GET request first
    const getResponse = await fetch("http://localhost:3005/api/ip-hash/method-test");
    ...
    const getResponse2 = await fetch("http://localhost:3005/api/ip-hash/method-test");
  ...
  test("should handle path rewriting correctly with IP-hash strategy", async () => {
    const response = await fetch("http://localhost:3005/api/ip-hash/custom/path");
    expect(response.status).toBe(200);
  ...
  test("should provide consistent routing for user sessions", async () => {
    ...
    for (const req of sessionRequests) {
      const response = await fetch(`http://localhost:3005/api/ip-hash${req.path}`, {
        method: req.method,
      });
