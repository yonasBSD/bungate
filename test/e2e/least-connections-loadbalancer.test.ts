...
describe("Least Connections Load Balancer E2E Tests", () => {
  ...
  beforeAll(async () => {
    ...
    echoServer1 = Bun.serve({
      ...
      async fetch(req) {
        ...
        const delay = url.searchParams.get("delay") ? parseInt(url.searchParams.get("delay")!) : 0;
        if (delay > 0) {
    ...
    gateway.addRoute({
      ...
      loadBalancer: {
        ...
        targets: [{ url: "http://localhost:8110" }, { url: "http://localhost:8111" }, { url: "http://localhost:8112" }],
        healthCheck: {
          enabled: true,
          interval: 2000,
          timeout: 1000,
    ...
    gateway.addRoute({
      ...
      loadBalancer: {
        strategy: "least-connections",
        targets: [{ url: "http://localhost:8110" }, { url: "http://localhost:8111" }],
        healthCheck: {
          enabled: true,
          interval: 2000,
          timeout: 1000,
  ...
  test("should distribute requests using least-connections strategy", async () => {
    ...
    for (let i = 0; i < requestCount; i++) {
      const response = await fetch("http://localhost:3003/api/least-connections/test");
    ...
    const totalRequests = (serverCounts["echo-1"] || 0) + (serverCounts["echo-2"] || 0) + (serverCounts["echo-3"] || 0);
  ...
  test("should balance load when servers have different response times", async () => {
    ...
    for (let i = 0; i < 9; i++) {
      ...
      promises.push(
        fetch(`http://localhost:3003/api/least-connections/mixed?delay=${delay}`)
          .then((res) => res.json())
          .then((data) => {
            const response = data as EchoResponse;
            if (response.server in serverCounts) {
              serverCounts[response.server] = (serverCounts[response.server] || 0) + 1;
            }
            return response;
          })
      );
    ...
    // Total should equal request count
    expect((serverCounts["echo-1"] || 0) + (serverCounts["echo-2"] || 0) + (serverCounts["echo-3"] || 0)).toBe(9);
  });

  test("should handle path rewriting correctly with least-connections strategy", async () => {
    const response = await fetch("http://localhost:3003/api/least-connections/custom/path");
    expect(response.status).toBe(200);
  ...
  test("should handle request headers correctly with least-connections strategy", async () => {
    const response = await fetch("http://localhost:3003/api/least-connections/headers", {
      headers: {
