  ...
  describe('Route Level Middleware', () => {
    it('should apply route-specific middleware only to specific routes', async () => {
      ...
      const protectedResponse = await fetch(
        'http://localhost:9003/api/protected/users',
      )
      ...
      const publicResponse = await fetch(
        'http://localhost:9003/api/public/users',
      )
      ...
      expect(routeSpecificLogs).toContain(
        'Route middleware: /api/protected/users',
      )
      expect(routeSpecificLogs).not.toContain(
        'Route middleware: /api/public/users',
      )
    ...
    it('should handle route-specific middleware with validation and transformation', async () => {
      ...
      const getResponse = await fetch(
        'http://localhost:9005/api/validated/users',
      )
      ...
      const validPostResponse = await fetch(
        'http://localhost:9005/api/validated/users',
        {
          method: 'POST',
          headers: {
