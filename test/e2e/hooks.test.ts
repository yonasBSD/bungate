...
describe("Hooks E2E Tests", () => {
  ...
  test("should use fallback response from onError hook when returned", async () => {
    ...
    try {
      const response = await fetch(`http://localhost:${fallbackGatewayPort}/api/fallback/error`);
      expect(response.status).toBe(200);
  ...
  test("should use fallback response from onError hook on timeout", async () => {
    ...
    try {
      const response = await fetch(`http://localhost:${timeoutGatewayPort}/api/timeout-fallback/timeout`);
      ...
      expect(timeoutErrorCalls[0]?.req.url).toContain("/api/timeout-fallback/timeout");
  ...
  test("should fallback to default error handling when onError hook throws", async () => {
    ...
    try {
      ...
      const timeoutResponse = await fetch(`http://localhost:${selectiveGatewayPort}/api/selective/timeout`);
  ...
  test("should handle async fallback response generation", async () => {
    ...
    try {
      ...
      expect(asyncErrorCalls[0]?.req.url).toContain(`requestId=${testRequestId}`);
    } finally {
      asyncServer.stop();
