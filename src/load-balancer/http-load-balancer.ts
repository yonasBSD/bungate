...
import type {
  ...
  LoadBalancerTarget,
  LoadBalancerStats,
} from "../interfaces/load-balancer.ts";
import type { Logger } from "../interfaces/logger.ts";
import { defaultLogger } from "../logger/pino-logger.ts";
...
export class HttpLoadBalancer implements LoadBalancer {
  ...
  getStats(): LoadBalancerStats {
    ...
    for (const [url, target] of this.targets.entries()) {
      targetStats[url] = {
        ...
        averageResponseTime: target.requests > 0 ? target.totalResponseTime / target.requests : 0,
  ...
  private selectIpHash(request: Request, targets: LoadBalancerTarget[]): LoadBalancerTarget {
    if (targets.length === 0) {
  ...
  private createStickySession(request: Request, target: LoadBalancerTarget): void {
    if (!this.config.stickySession?.enabled) {
      return;
    }
  ...
  private async performHealthChecks(): Promise<void> {
    ...
    const promises = Array.from(this.targets.values()).map(async (target: LoadBalancerTarget) => {
      const startTime = Date.now();
      try {
        ...
        const isHealthy = response.status === (healthCheckConfig.expectedStatus ?? 200);
        ...
        this.logger.logHealthCheck(target.url, false, duration, error as Error);
      }
    });
 ...
 */
export function createLoadBalancer(config: LoadBalancerConfig): HttpLoadBalancer {
